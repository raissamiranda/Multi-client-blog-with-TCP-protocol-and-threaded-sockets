...
int main(int argc, char **argv) {
  ...
  struct BlogOperation operationToSendByClient =
      createOperation(0, NEW_CONNECTION, 0, "", "");

  // Send connection request to server
  size_t size = send(sockfd, &operationToSendByClient,
                     sizeof(operationToSendByClient), 0);
  ...
  size = receive_all(sockfd, &operationReceivedByServer,
                     sizeof(operationReceivedByServer));
  ...
  while (1) {
    ...
    case NEW_POST_IN_TOPIC:
      ...
      operationToSend = createOperation(operationReceivedByServer.client_id,
                                        NEW_POST_IN_TOPIC, 0, topic, content);
      break;
    ...
    case SUBSCRIBE_IN_TOPIC:
      topic = getTopic(commandType, command);
      operationToSend = createOperation(operationReceivedByServer.client_id,
                                        SUBSCRIBE_IN_TOPIC, 0, topic, "");
      break;

    case UNSUBSCRIBE_IN_TOPIC:
      topic = getTopic(commandType, command);
      operationToSend = createOperation(operationReceivedByServer.client_id,
                                        UNSUBSCRIBE_IN_TOPIC, 0, topic, "");
      break;
  ...
  for (int i = 0; i < sizeof(commands) / sizeof(struct Command); i++) {
    ...
    if (strncmp(input, commands[i].name, strlen(commands[i].name)) == 0) {
      ...
      if (strcmp(command, "subscribe") == 0 ||
        ...
        return (strcmp(command, "subscribe") == 0) ? SUBSCRIBE_IN_TOPIC
                                                   : UNSUBSCRIBE_IN_TOPIC;
      }
...
void *waitingFunction(void *sock) { // socket do cliente
  ...
  while (1) {
    struct BlogOperation operationReceivedByServer;
    // Receive response from server
    size_t size = receive_all(*sockfd, &operationReceivedByServer,
                              sizeof(operationReceivedByServer));
    if (size != sizeof(operationReceivedByServer)) {
      logexit("receive");
    }
    ...
    case NEW_POST_IN_TOPIC:
      printf("new post added in %s by 0%d\n%s", operationReceivedByServer.topic,
             operationReceivedByServer.client_id + 1,
